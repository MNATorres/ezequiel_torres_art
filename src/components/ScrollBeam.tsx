...
function Beam({ side }: { side: "left" | "right" }) {
  ...
  const pos = side === "left" ? "left-4 md:left-10" : "right-4 md:right-10";
  ...
  return (
    <div className={`pointer-events-none fixed inset-y-0 z-40 w-px ${pos}`}>
      ...
      {/* bright travelling head + soft halo */}
      <motion.div
        style={{ top: headTop }}
        className="absolute left-1/2 -translate-x-1/2 -translate-y-1/2"
      >
        <div className="h-24 w-px bg-gradient-to-b from-transparent via-white to-transparent shadow-[0_0_22px_7px_rgba(255,255,255,0.85)]" />
        <div className="absolute left-1/2 top-1/2 h-28 w-28 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white/15 blur-3xl" />
      </motion.div>
    </div>
  );
}
